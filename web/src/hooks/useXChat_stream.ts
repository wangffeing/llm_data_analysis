  ...
  const handleSSEMessage = useCallback((sseMessage: SSEMessage) => {
    ...
    switch (sseMessage.type) {
        ...
        setThoughtSteps(prev => {
          const updated = prev.map(step => {
            if (step.id === sseMessage.data.post_id) {
              ...
              // 处理增量附件更新
              if (sseMessage.data.type === "attachment_increment") {
                ...
                if (existingIndex >= 0) {
                  ...
                  newAttachments[existingIndex] = {
                    ...newAttachments[existingIndex],
                    content: (newAttachments[existingIndex].content || '') + attachment.content
                  };
                } else {
                  // 创建新附件
                ...
                switch (attachment.type) {
                  case 'plan_reasoning':
                    ...
                    // 获取累加后的完整内容
                    const fullContent = newAttachments.find(a => a.id === attachment.id)?.content || '';
                    updatedStep.description = updateDescription(
                      config.descriptionMode === 'replace' ? undefined : step.description,
                      fullContent,
                      '**推理过程：**\n'
                    );
                    break;
                  case 'plan':
                    ...
                    const fullPlanContent = newAttachments.find(a => a.id === attachment.id)?.content || '';
                    updatedStep.description = updateDescription(
                      config.descriptionMode === 'replace' ? undefined : step.description,
                      fullPlanContent,
                      '**计划制定：** '
                  ...
                  case 'current_plan_step':
                    ...
                    const fullStepContent = newAttachments.find(a => a.id === attachment.id)?.content || '';
                    updatedStep.description = updateDescription(
                      config.descriptionMode === 'replace' ? undefined : step.description,
                      fullStepContent,
                      '**当前步骤：**\n'
      ...
      case MessageType.POST_MESSAGE_UPDATE:
        if (sseMessage.data.type === "message_increment") {
          // 确保只更新 assistant 消息，不更新用户消息
          setMessages(prev => prev.map(msg => {
            // 只更新最后一条 assistant 消息，而不是基于 currentMessageRef
            if (msg.role === 'assistant' && msg.id === prev[prev.length - 1]?.id) {
              return {
                ...msg,
                content: (msg.content || '') + sseMessage.data.content,
                status: 'streaming' as const
              };
            }
            return msg;
          }));
        } else if (sseMessage.data.is_complete) {
          // 处理完整消息更新（保持原有逻辑）
          setThoughtSteps(prev => {
            const updated = prev.map(step => {
              if (step.id === sseMessage.data.post_id) {
